...
from .peft_cara import CaraLinear
...
class MHE_CARA(nn.Module):
    def __init__(self, model, adapter_name="default"):
        super(MHE_CARA, self).__init__()
        ...
        with torch.no_grad():
            for name, module in model.named_modules():
                if isinstance(module, CaraLinear):
                    self.extracted_params[f"{name}.weight"] = module.get_effective_weight(adapter_name).detach().clone()

    def calculate_mhe(self):
        mhe_loss = []
        with torch.no_grad():
            for weight in self.extracted_params.values():
                if len(weight.shape) == 2 or len(weight.shape) == 4:
                    loss = MHE_db.mhe_loss(weight)
                    mhe_loss.append(loss.cpu().detach().item())
            mhe_loss = np.array(mhe_loss)
        return mhe_loss.sum()
